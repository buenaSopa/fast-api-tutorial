...
def delete_posts(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if not post_query.first():
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="no content in database")
...
def update_posts(id: int, updated_post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    db.commit()
    return post_query.first() #updated_post.dict() doesnt work because request schema not same as response schema